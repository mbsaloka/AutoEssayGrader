...
function PesertaContent() {
  ...
  const handleRemoveParticipant = async (userId: number, userName: string) => {
    if (window.confirm(`Apakah Anda yakin ingin menghapus ${userName} dari kelas ini?`)) {
      try {
          ...
          <div className="space-y-2 sm:space-y-3">
            {participants.map((participant) => (
              ...
              >
                ...
                {isTeacher && participant.user_id !== classData.teacher_id && (
                  <button
                    onClick={() => handleRemoveParticipant(participant.user_id, participant.fullname)}
